...
class ViewController: UIViewController, UITextFieldDelegate, SFSafariViewControllerDelegate
{
    ...
    @IBAction func mvpButtonPressed(_ sender: UIButton)
    {
        ...
        // MARK: - MVP
        let equalTo:Bool = number1 == number2
        let notEqualTo:Bool = number3 != number4
        let greaterThan:Bool = number5 > number6
        let lessThan:Bool = number7 < number8
        let greaterThanOrEqualTo:Bool = number9 >= number10
        let lessThanOrEqualTo:Bool = number11 <= number12
    ...
    @IBAction func stretchOne(_ sender: UIButton)
    {
        ...
        // MARK: - Stretch #1
        let purchasedPackages:Int = tickets / 4
        let individualTickets:Int = tickets % 4
    ...
    @IBAction func whenPinched(_ sender: UIPinchGestureRecognizer) {
        ...
        if sender.velocity > 0 { tickets += 1
